...
if (import.meta.vitest) {
  ...
  let A = Component.value()
  let B = Component.value()
  let C = Component.value()
  let D = Component.value()
  let AB = Type.make(A, B)
  let BC = Type.make(B, C)
  let ABC = Type.make(A, B, C)
  describe("Graph", () => {
    it("inserts nodes", () => {
      let graph = make()
    ...
    it("unlinks nodes adjacent to deleted ones", () => {
      let graph = make()
      let node_a = resolve(graph, A)
      let node_b = resolve(graph, B)
      let node_c = resolve(graph, C)
      let node_abc = resolve(graph, ABC)
    ...
    it("emits inserted signal", () => {
      ...
      let node_c = resolve(graph, C)
      let node_ab = resolve(graph, AB)
      let visited = {a: [], b: [], c: [], ab: []} as Record<string, Node[]>
