...
export let makeConfig = (config: Partial<Config>): Config => {
  return {
...
export let makeConfigFromEnv = (): Config => {
  let envPerfUnit = process.env.PERF_UNIT
  if (envPerfUnit !== undefined) {
    assertPerfUnitValid(envPerfUnit)
  }
  return makeConfig({
